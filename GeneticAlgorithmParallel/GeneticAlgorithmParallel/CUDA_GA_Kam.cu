...
//}
//
//__global__ void geneticAlgorithmKernel(char* d_population, char* d_child, size_t itemCount, int* d_totalWeight, int* d_fitness, const Item* d_items, double mutationRate, int* d_bestFitness) {
...
//        }
//
//        // Reduce the child's fitness if its total weight exceeds the knapsack capacity
//        if (totalWeight > KNAPSACK_CAPACITY) {
//            d_fitness[threadID] -= (totalWeight - KNAPSACK_CAPACITY) * 10;
//        }
//        else {
...
//
//        // Update the best fitness if the child has a better fitness
//        if (d_fitness[threadID] > bestFitness) {
//            bestFitness = d_fitness[threadID];
//        }
//       
//    }
//
...
//    cudaMalloc((void**)&d_bestFitness, 1 * sizeof(int));
//
...
//
//    //copying best fitness to device
...
//        cudaMemcpy(d_bestFitness, &bestFitness, sizeof(int), cudaMemcpyHostToDevice);
//        //calling kernel, passing best fitness as a parameter
//        geneticAlgorithmKernel << <1, 128 >> > (d_population, d_child, itemCount, d_totalWeight, d_fitness, d_items, MUTATION_RATE, d_bestFitness);
//
